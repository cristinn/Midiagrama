...
public class Ejecutable {
	...
	public static void main(String[] args) {
		...
		int maxtemp = 0, postemp=0; // variable que guardará el número máximo de temporadas
		...
		System.out.println(matSeries[postemp].toString());
			
			
		
		//Mostrar la película del año más reciente:
		
		int maxanio=0, posanio =0; // variable que guardará el año más reciente (es decir, el año mayor)
		 
		
		for(int k=0;k<5;k++){
			if(matPeliculas[k].getAño()>maxanio){
				maxanio = matPeliculas[k].getAño();
			}
			posanio = k;
		}
				System.out.println("\nLa película del año más reciente es: ");
				System.out.println(matPeliculas[posanio].toString());
